...
- ✅ **Sep 8 Session — Shoot Dead / Mark Dead Account Eradication Fix:**
  - **Account Tombstones Migration (`062_account_tombstones_and_triggers.sql`):** Created `public.account_tombstones` table, RLS policies, PostgREST grants, and attached `tr_enforce_tombstone_<table_name>` triggers across all 34 user-owned tables. Applied to production Supabase and verified schema cache reloaded.
  - **AccountLifecycleService Hardening (`AccountLifecycleService.ts`):** Removed premature Step 0 bail-out. Data wipe continues and records error into audit log even if tombstone logging encounters an issue.
  - **Mobile 401 Refresh Interceptor (`mobile/src/services/api.ts`):** Narrowed skip list to unauthenticated routes (`/auth/login`, `/auth/signup`, `/auth/refresh`), allowing protected endpoints like `DELETE /auth/mark-dead` to auto-refresh expired JWTs instead of failing immediately.
  - **Settings UI Quality (`SettingsScreen.tsx`):** Displays exact server error in alert dialog if eradication fails.
  - **Tests & Verification:** 4/4 route tests and 10/10 concurrency tests passing (`shoot_dead.test.ts`, `shoot_dead_concurrency.test.ts`). End-to-end verified on live Supabase.
