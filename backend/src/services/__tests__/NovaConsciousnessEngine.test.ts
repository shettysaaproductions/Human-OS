...
describe('NovaConsciousnessEngine — sleep/busy lock respect', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    // Fresh chainable per test (see NovaFollowupService.test.ts — same pattern).
  ...
  it('does not abort when the lock is expired', async () => {
    // profiles.fetch → has push_token
    mockChain.maybeSingle.mockResolvedValueOnce({ data: { push_token: 'tok-123', preferred_name: 'x', timezone_offset: 330 } });
    // followup_suppressed_until → in the PAST (lock expired)
