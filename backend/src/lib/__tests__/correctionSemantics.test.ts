...
describe('correctionSemantics validation', () => {
  ...
  it('accepts favourite_color / green from the production sentence meaning', () => {
    const validated = validateSemanticCorrection(
      { key: 'favourite color', value: 'green' },
      productionSentence
    );
  ...
  it('never persists a value-derived key such as favourite_color_green', () => {
    ...
    expect(validateSemanticCorrection(
      { key: 'favourite_color_green', value: 'ab' },
      productionSentence
    )).toBeNull();

    expect(validateSemanticCorrection(
      { key: 'favourite_color_green', value: 'green' },
      productionSentence
    )).toBeNull();
  });
  ...
  it('rejects values not grounded in the source message', () => {
    ...
    expect(validateSemanticCorrection(
      { key: 'favourite_color', value: 'purple' },
      productionSentence
    )).toBeNull();
  });
  ...
  it('rejects invented / unknown targets', () => {
    expect(validateSemanticCorrection(
      { key: 'spaceship_model', value: 'green' },
      productionSentence
    )).toBeNull();
  });

  it('fails closed on missing target or value', () => {
    expect(validateSemanticCorrection({ key: 'favourite_color', value: '' }, productionSentence)).toBeNull();
    expect(validateSemanticCorrection({ key: '', value: 'green' }, productionSentence)).toBeNull();
  ...
  it.each([
    ...
    const validated = validateSemanticCorrection(
      { key: 'favourite_color', value: expectedValue },
      source
    );
  ...
  it('selectAuthoritativeCorrections drops malformed TurnAnalyzer-style rows', () => {
