-- Migration 20260918: Add bubble_id to kg_nodes and create atomic canonical_merge_entities RPC

-- 1. Add bubble_id to kg_nodes (foreign-keyed to memory_bubbles.id)
ALTER TABLE public.kg_nodes ADD COLUMN IF NOT EXISTS bubble_id UUID REFERENCES public.memory_bubbles(id) ON DELETE SET NULL;
CREATE INDEX IF NOT EXISTS idx_kg_nodes_user_bubble ON public.kg_nodes(user_id, bubble_id) WHERE bubble_id IS NOT NULL;
CREATE UNIQUE INDEX IF NOT EXISTS idx_kg_nodes_user_bubble_unique ON public.kg_nodes(user_id, bubble_id) WHERE bubble_id IS NOT NULL;

-- 2. Atomic Database Transaction Function for Canonical Entity Merging
...
BEGIN
  ...
  -- 2. Lock both bubbles for update (durable transaction isolation)
  ...
  FOR v_rel_item IN SELECT * FROM jsonb_array_elements(v_source_rels) LOOP
    IF (v_rel_item->>'targetEntityId')::uuid <> p_target_bubble_id AND
       NOT EXISTS (
         SELECT 1 FROM jsonb_array_elements(v_merged_rels) existing
         WHERE existing->>'targetEntityId' = v_rel_item->>'targetEntityId'
       ) THEN
      v_merged_rels := v_merged_rels || jsonb_build_array(v_rel_item);
    END IF;
  END LOOP;

  -- 7. Repoint incoming relationships from other bubbles pointing to source
      ...
      SELECT coalesce(jsonb_agg(
        CASE 
          ...
          THEN jsonb_build_object(
            'relationshipId', coalesce(r->>'relationshipId', gen_random_uuid()::text),
  ...
  -- 12. Repoint kg_nodes & kg_edges read projection
  ...
  IF v_source_kg_node.id IS NOT NULL AND v_target_kg_node.id IS NOT NULL AND v_source_kg_node.id <> v_target_kg_node.id THEN
    UPDATE public.kg_edges SET source_node_id = v_target_kg_node.id, updated_at = v_now WHERE user_id = p_user_id AND source_node_id = v_source_kg_node.id;
    UPDATE public.kg_edges SET target_node_id = v_target_kg_node.id, updated_at = v_now WHERE user_id = p_user_id AND target_node_id = v_source_kg_node.id;
