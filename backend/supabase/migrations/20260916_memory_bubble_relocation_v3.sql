...
begin
 ...
 if v_target_parent is not null then
   ...
   if v_target_label is not null and v_target_slug<>'' and v_target_slug<>lower(p_target_domain) then
     ...
     if v_target_parent is null then insert into memory_bubbles(user_id,parent_bubble_id,label,slug,bubble_type,domain_key,metadata,created_at,updated_at) values(p_user_id,v_domain_bubble,v_target_label,'branch:'||v_target_slug,'branch',lower(p_target_domain),null,jsonb_build_object('created_for_move',true),v_now,v_now) returning id into v_target_parent; end if;
   end if;
 end if;
