-- V2: reparent an existing bubble subtree to any target bubble atomically.
-- The live function uses a transaction-scoped user lock plus stale-proposal checks.

create or replace function public.move_memory_branch_atomic_v2(
  ...
  p_target_parent_bubble_id uuid default null, p_confirmation_fingerprint text default null,
...
declare
 ...
 v_target_parent uuid:=p_target_parent_bubble_id; v_source_bubble uuid; v_entity_bubble uuid;
...
begin
 ...
 if v_target_parent is not null then
   ...
   select id into v_target_parent from memory_bubbles where user_id=p_user_id and parent_bubble_id is null and slug='domain:'||lower(p_target_domain) and is_archived=false limit 1;
   if v_target_parent is null then
     insert into memory_bubbles(user_id,parent_bubble_id,label,slug,bubble_type,domain_key,metadata,created_at,updated_at)
     values(p_user_id,null,initcap(replace(lower(p_target_domain),'_',' ')),'domain:'||lower(p_target_domain),'domain',lower(p_target_domain),'{}',v_now,v_now) returning id into v_target_parent;
   end if;
 end if;
 ...
 if coalesce(array_length(p_reminder_ids,1),0)>0 then
   select count(*) into v_rem_count from reminders where user_id=p_user_id and id=any(p_reminder_ids) and status not in('cancelled','completed');
   if v_rem_count<>array_length(p_reminder_ids,1) then raise exception 'REMINDER_SET_CHANGED'; end if;
 end if;
...
revoke all on function public.move_memory_branch_atomic_v2(uuid,uuid[],uuid[],text,text,text,text,text,uuid,text,jsonb) from public;
grant execute on function public.move_memory_branch_atomic_v2(uuid,uuid[],uuid[],text,text,text,text,text,uuid,text,jsonb) to service_role;
